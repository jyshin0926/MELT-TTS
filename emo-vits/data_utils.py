...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        if os.path.exists(len_pkl):
            ...
            lengths = []
            for audiopath, sid, text, emo, sst in tqdm(self.audiopaths_sid_text):
                if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
                    audiopaths_sid_text_new.append([audiopath, sid, text, emo, sst])
    ...
    def get_emotion(self, emotion):
        emotion = torch.LongTensor([int(emotion)])
        return emotion
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        _, ids_sorted_decreasing = torch.sort(
            torch.LongTensor([x[1].size(0) if x[1].dim() == 1 else x[1].size(1) for x in batch]),
            # torch.LongTensor([x[1].size(0) for x in batch]),
            dim=0, descending=True)

        max_text_len = max([len(x[0]) for x in batch])
        # max_spec_len = max([x[1].size(1) for x in batch])
        max_spec_len = max([x[1].size(0) if x[1].dim() == 1 else x[1].size(1) for x in batch])
        ...
        emotion = torch.LongTensor(len(batch))
        sensitivity = torch.LongTensor(len(batch))
        ...
        return text_padded, text_lengths, spec_padded, spec_lengths, wav_padded, wav_lengths, sid, emotion, sensitivity
